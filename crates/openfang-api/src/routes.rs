...
pub async fn test_provider(
    ...
    let (env_var, base_url, key_required, default_model, protocol_type) = {
        let catalog = state
            .kernel
            .model_catalog
            .read()
...
pub async fn set_provider_url(
    ...
    // Update catalog in memory
    {
        ...
        catalog.set_provider_protocol(&name, &protocol_type);
    }
    ...
    if let Err(e) = upsert_provider_protocol(&config_path, &name, &protocol_type) {
        return (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(serde_json::json!({"error": format!("Failed to save protocol: {e}")})),
        );
    }

    // Probe reachability for OpenAI-compatible endpoints only.
    let probe = if protocol_type == "openai" {
        openfang_runtime::provider_health::probe_provider(&name, &base_url).await
    } else {
        openfang_runtime::provider_health::ProbeResult {
            reachable: false,
            latency_ms: 0,
...
fn upsert_provider_protocol(
    ...
    let root = doc.as_table_mut().ok_or("Config is not a TOML table")?;
    if !root.contains_key("provider_protocols") {
        root.insert(
            "provider_protocols".to_string(),
            toml::Value::Table(toml::map::Map::new()),
        );
    }

    let protocols_table = root
        .get_mut("provider_protocols")
        .and_then(|value| value.as_table_mut())
        .ok_or("provider_protocols is not a table")?;

    protocols_table.insert(
        provider.to_string(),
        toml::Value::String(protocol_type.to_string()),
    );
